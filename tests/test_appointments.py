...
def appointment_data():
    ...
    future_date = (datetime.now(timezone.utc) + timedelta(days=1)).replace(microsecond=0)
    return {
        "user_id": 1,
        "car_id": 1,
        "service_id": 1,
...
@pytest.mark.asyncio
async def test_create_appointment(async_session: AsyncSession, appointment_data):
    ...
    assert appointment.appointment_date == appointment_data["appointment_date"].replace(tzinfo=None)
...
async def test_read_appointment(async_session: AsyncSession, appointment_data):
    ...
    fetched_appointment = await async_session.get(Appointment, appointment.appointment_id)
...
@pytest.mark.asyncio
async def test_update_appointment(async_session: AsyncSession, appointment_data):
    ...
    updated_appointment = await async_session.get(Appointment, appointment.appointment_id)
    assert updated_appointment.appointment_date == new_date
    assert updated_appointment.status == AppointmentStatus.COMPLETED


@pytest.mark.asyncio
async def test_delete_appointment(async_session: AsyncSession, appointment_data):
    ...
    deleted_appointment = await async_session.get(Appointment, appointment.appointment_id)
